...
OGLCONSOLE_Console OGLCONSOLE_Create()
{
    ...
    console->characterWidth = (double)(CHAR_PIXEL_W) / viewport[2];
    console->characterHeight = (double)(CHAR_PIXEL_H) / viewport[3];
    ...
    console->linesColors = (GLfloat*)malloc(console->maxLines*3*sizeof(GLfloat));
...
//    glBlendFunc(GL_ONE_MINUS_DST_COLOR, GL_ONE_MINUS_SRC_COLOR);
    ...
    glBegin(GL_QUADS);
    {
        ...
        for (gLine = 0; gLine < C->textHeight; gLine++)
        {
            ...
            /* Recolor text */
            glColor3fv(C->linesColors + (tLine * 3));
...
void OGLCONSOLE_Output(OGLCONSOLE_Console console, const char *s, ...)
{
    ...
    OGLCONSOLE_ColorOutput((OGLCONSOLE_Console)userConsole, 0,1,0, "%s", output);
}

/* This is the final, internal function for printing text to a console */
void OGLCONSOLE_ColorOutput(OGLCONSOLE_Console console, GLfloat r, GLfloat g, GLfloat b, const char *s, ...)
{
    va_list argument;
    ...
    while (*outputCursor)
    {
                ...
                >= (textWidth - 1))
        {
            ...
            C->linesColors[lineQueueIndex * 3 + 0] = r;
            C->linesColors[lineQueueIndex * 3 + 1] = g;
            C->linesColors[lineQueueIndex * 3 + 2] = b;
        }
...
int OGLCONSOLE_SDLEvent(SDL_Event *e)
{
    ...
    if (userConsole->visibility < 1)
    {
        if (e->type == SDL_KEYDOWN && e->key.keysym.sym == KEY_HIDE)
        {
    ...
    if (e->type == SDL_KEYDOWN)
    {
        ...
        else if (e->key.keysym.sym == KEY_RETURN)
        {
            ...
            OGLCONSOLE_ColorOutput((void*)userConsole, 0.5, 1, 0.5, "%s\n", userConsole->inputLine);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (--userConsole->historyScrollIndex < 0)
                        userConsole->historyScrollIndex = MAX_HISTORY_COUNT-1;
                }
