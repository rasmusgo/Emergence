...
int ClientManager::server_thread_func()
{
	while (!done)
	{
		...
		if (need_to_load)
		{
			...
			else
			{
				...
				for (typeof(clients.begin()) it = clients.begin();
						it != clients.end(); ++it)
				{
		...
		for (typeof(clients.begin()) it = clients.begin(); it != clients.end(); ++it)
		{
			...
			// Send entities
			for (typeof(world->get_entities().begin()) e =
					world->get_entities().begin();
					e != world->get_entities().end(); ++e)
	...
	// Close all sockets
	for (typeof(clients.begin()) it = clients.begin(); it != clients.end();
			++it)
		delete *it;
	clients.clear();
	return 0;
...
void ClientManager::add_client(TCPsocket new_tcpsock)
{
	...
	// Create new client
	ClientConnection *client_connection = new ClientConnection(new_tcpsock,
			this);
