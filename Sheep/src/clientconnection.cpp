...
namespace WolfSheepServer
{
    ...
    void ClientConnection::send(const void *data, int len)
    {
        ...
        DataPacket packet = {len, new char[len]};
    ...
    void ClientConnection::send_entity(Uint32 eid, const Entity *ent)
    {
        DataEntity dent = {
            eid,
            ent->world->difference(ent->get_x(), get_x())*64 + ent->get_offset_x(),
        ...
        if (it != client_entities.end())
        {
            ...
            // Check bounds
            if ( (dent.x + dent.w < -1024 || dent.x > 1024 ||
                  dent.y + dent.h < -1024 || dent.y > 1024) &&
    ...
    int ClientConnection::recieve_thread_func()
    {
        while (status == 0)
        {
            ...
            ReceivedPacket packet = { key, released, arrival_time };
