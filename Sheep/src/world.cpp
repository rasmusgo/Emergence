...
namespace WolfSheepServer
{
    ...
    void World::get_entities_on(int x, int y, std::vector<Entity*> &container) const
    {
        ...
        {
            if ( difference(e->second->get_x(), x) == 0 && difference(e->second->get_y(), y) == 0 )
    ...
    bool World::is_blocked(int x, int y) const
    {
        for (typeof(entities.begin()) e = entities.begin(); e != entities.end(); ++e)
        {
            if ( difference(e->second->get_x(), x) == 0 && difference(e->second->get_y(), y) == 0 && e->second->is_blocking() )
                return true;
        }
        return false;
    }
    ...
    std::ostream& operator << (std::ostream &os, World &w)
    {
        ...
        // Entities
        for (typeof(w.entities.begin()) e = w.entities.begin(); e != w.entities.end(); ++e)
        {
    ...
    std::istream& operator >> (std::istream &is, World &w)
    {
        ...
        for (typeof(secondpass.begin()) e = secondpass.begin(); e != secondpass.end(); ++e)
        {
